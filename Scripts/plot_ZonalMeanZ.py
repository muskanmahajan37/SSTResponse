"""
Script plots the zonal mean zonal geopotential response across all latitudes.
...
directoryfigure = '/home/zlabe/Desktop/SSTVARI/ZonalMeanZ/'
...
### Read in data
anomy,climoy,pvaly,lat,lon,lev = readDataPeriods('GEOP',['SST_Fu','SST_Cu'],'Annual')
anomwi,climowi,pvalwi,lat,lon,lev = readDataPeriods('GEOP',['SST_Fu','SST_Cu'],'JFM')
anomsu,climosu,pvalsu,lat,lon,lev = readDataPeriods('GEOP',['SST_Fu','SST_Cu'],'JJA')
...
limit = np.arange(-200,200.1,1)
barlim = np.arange(-200,250,50)
...
for i in range(len(data)):
    ...
    cs1 = plt.contour(lat,lev,climo,np.arange(0,36000,2000),
...
cbar.set_label(r'\textbf{Z [m]}',fontsize=11,color='dimgray')
...
plt.savefig(directoryfigure + 'Z_zonalmean.png',dpi=300)